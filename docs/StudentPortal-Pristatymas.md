...
Studentų sistema (StudentPortal) – tai išsami švietimo valdymo sistema, sukurta palengvinti studentų, dėstytojų ir administratorių bendravimą. Sistema leidžia efektyviai valdyti vartotojų profilius, peržiūrėti ir atnaujinti akademinius įrašus, planuoti užsiėmimus ir atlikti administracines operacijas.
...
- Dėstytojų sąsaja valdyti studentų pažymius
- Administratoriaus panelė vartotojų valdymui ir sistemos nustatymams
...
2. **Back-End dalis**: .NET Core API, kuri tvarko duomenų apdorojimą, logiką ir duomenų bazės operacijas
...
- **Entity Framework Core 9**: ORM (Object Relational Mapping) duomenų bazės operacijoms
- **SQL Server**: Reliacinė duomenų bazė
- **JWT Authentication**: JSON Web Token saugioms vartotojų sesijoms
- **BCrypt**: Slaptažodžių šifravimui (password-hashing)
...
Sistema naudoja reliacinį duomenų modelį, kuris apima šitas pagrindines esybes:

### User (Vartotojas)
- Id (int)
- Email (string)
- PasswordHash (string)
- Role (string) - nurodo vartotojo tipą (Student, Teacher, Admin)
- Name (string)
- Student (Student)
...
### Grade (Pažymys)
- Id (int)
- StudentId (int) - ryšys su Student lentele (One student - many grades) 
- Subject (string)
- Score (decimal)
- MaxScore (decimal)
- GradeType (string)
...
1. **Prisijungimo puslapis**
   - Formos validacija
   - Klaidų apdorojimas
   - Nukreipimas į prisijungusio vartotojo atitinkamą puslapį (Student, Teacher, Admin)
...
2. Rolėmis pagrįsta prieigos kontrolė naudojant tokenus
...
### Back-end diegimas
1. Naviguokite į API projekto dir:
   ```
...
2. Sukurkite konfigūracijos failus:
   ...
   - Atnaujinkite duomenų bazės prisijungimus ir JWT nustatymus abiejuose failuose

3. Altikite duomenų bazės migracijas:
   ```
...
### Front-end diegimas
1. Naviguokite į front-end projekto dir:
   ```
   cd student-portal
   ```
